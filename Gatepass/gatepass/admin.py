        ...
        try:
            ...
            if request.method == 'POST':
                ...
                try:
                    ...
                    # Import students (all valid rows are saved; invalid rows are skipped)
                    importer = StudentBulkImporter(tmp_path)
                    importer.import_students()
                    from django.http import HttpResponseRedirect

                    if len(importer.successes) > 0:
                        messages.success(
                            request,
                            f'✓ {len(importer.successes)} student(s) imported successfully and saved to the database.'
                        )
                        if importer.errors:
                            messages.warning(
                                request,
                                f'{len(importer.errors)} row(s) were skipped due to errors or duplicates.'
                            )
                        return HttpResponseRedirect('../')
                    elif importer.errors:
                        # No rows succeeded; show results with errors only
                        context = {
                            'title': 'Bulk Import Results - Students',
                            'successes': importer.successes,
                            'errors': importer.errors,
                            'total_rows': len(importer.errors),
                            'success_count': 0,
                        ...
                        messages.warning(request, 'No data found in the file to import.')
                        return render(request, 'admin/gatepass/bulk_import.html', {'title': 'Bulk Import Students'})
