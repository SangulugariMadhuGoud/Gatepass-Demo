...
def security_dashboard(request):
    ...
    # Removed limit to display all records, even if count exceeds 200
...
def security_record_return(request, gatepass_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Capture exact return time automatically
            now = timezone.now()
        ...
        # Don't use form for return time - we capture it automatically
        # But we still need a form for return_notes if needed
        from .forms import SecurityReturnForm
