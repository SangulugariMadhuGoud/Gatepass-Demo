                ...
                with transaction.atomic():
                    for idx, row in batch_df.iterrows():
                        try:
                            ...
                            hall_ticket_no = str(row['Hall Ticket No']).strip()
                            room_no = str(row['Room No']).strip()
                            gender = str(row['Gender']).strip().upper()
                            email = str(row['Email']).strip()
                            mobile_number = str(row['Mobile']).strip()
                            parent_name = str(row['Parent Name']).strip()
                            parent_mobile = str(row['Parent Mobile']).strip()
                            
                            # Check if 'Approved' column exists and get its value
                            is_approved = True  # Default to True
                            if 'Approved' in self.df.columns:
                                approved_val = str(row.get('Approved', 'Yes')).strip().lower()
                                is_approved = approved_val in ['yes', 'true', '1', 'y']
                            
                            # Validate required fields
                            if not all([student_name, hall_ticket_no, room_no, parent_name, 
                                       parent_mobile, email, mobile_number, gender]):
